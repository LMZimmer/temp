...
def get_data_paths(surrogate_model):
    return surrogate_model.train_paths, surrogate_model.val_paths, surrogate_model.test_paths
...
def load_data(surrogate_model, result_paths):
    ...
    for result_path in tqdm(result_paths):
        config_space_instance, val_accuracy, test_accuracy, _ = surrogate_model.config_loader[result_path]
...
def change_config_fidelity(config, target_fidelity):
    ...
    config["SimpleLearningrateSchedulerSelector:cosine_annealing:T_max"] = epoch_fidelities[target_fidelity]
    config["NetworkSelectorDatasetInfo:darts:layers"] = cell_fiedelities[target_fidelity]
    config["NetworkSelectorDatasetInfo:darts:init_channels"] = channel_fidelities[target_fidelity]
...
def matrix_plot(matrix, plot_log_dir):
    ...
    for i in range(7):
        for j in range(7):
            if i == j:
                plt.text(j, i, '{:.2f}'.format(1), horizontalalignment='center', verticalalignment='center')
            else:
                plt.text(j, i, '{:.2f}'.format(matrix[i, j]), horizontalalignment='center', verticalalignment='center')
...
def plot_correlations(corr_matrix_true, corr_matrix_model, corr_matrix_random,
                      model_type, model_log_dir):
    matrix_plot(corr_matrix_true,
                plot_log_dir=os.path.join(model_log_dir, "fidelity_correlation_true_" + str(model_type) + ".pdf"))
    matrix_plot(corr_matrix_model, plot_log_dir=os.path.join(model_log_dir, "fidelity_correlation_model_noisy_" + str(
        model_type) + ".pdf"))
    matrix_plot(corr_matrix_random, plot_log_dir=os.path.join(model_log_dir, "fidelity_correlation_random_noisy_" + str(
        model_type) + ".pdf"))
...
def query_with_noise(surrogate_model, config, variances, fidelity):
    ...
    noise = np.random.normal(0, variances[fidelity], 1)
...
def fidelity_correlation(model_log_dir, nasbench_data, configspace_path):
    ...
    config_hashdict = ConfigDict(X, y, paths=None, include_off_diagonal=False, allow_repeats=True)
    ...
    # Get correlations (TODO: refactor upper and lower stuff)
    for start_from_fidelity_fidelity in range(7):
        ...
        for fidelity in range(7):
            ...
            # Correlation on data
            upper_configs, upper_accuracies, lower_configs, lower_accuracies = get_fidelity_configs_and_accuracies(
            ...
            model_upper_pred = [query_with_noise(surrogate_model, config, variances, start_from_fidelity_fidelity) for
                                config in upper_configs]
            model_lower_pred = [query_with_noise(surrogate_model, config, variances, fidelity) for config in
                                lower_configs]
            ...
            simulated_upper_configs = change_all_config_fidelities(copy.deepcopy(simulated_configs),
                                                                   start_from_fidelity_fidelity)
            simulated_lower_configs = change_all_config_fidelities(copy.deepcopy(simulated_configs), fidelity)
            model_simulated_upper_pred = [
                query_with_noise(surrogate_model, config, variances, start_from_fidelity_fidelity) for config in
                simulated_upper_configs]
            model_simulated_lower_pred = [query_with_noise(surrogate_model, config, variances, fidelity) for config in
                                          simulated_lower_configs]
            model_simulated_corr, p_coeff = stats.kendalltau(model_simulated_upper_pred, model_simulated_lower_pred)
            ...
            corr_matrix_random[start_from_fidelity_fidelity, fidelity] = model_simulated_corr
            print("From %i to %i: (%f, %f, %f)" % (
                start_from_fidelity_fidelity, fidelity, real_corr, model_corr, model_simulated_corr))

    plot_correlations(corr_matrix_true, corr_matrix_model, corr_matrix_random,
                      model_config['model'], model_log_dir)
