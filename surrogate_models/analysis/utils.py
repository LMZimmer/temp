...
class ConfigDict():
    ...
    def __init__(self, configs, accuracies, paths=None, include_off_diagonal=False, allow_repeats=False):
        self.fidelity_keys = ["SimpleLearningrateSchedulerSelector:cosine_annealing:T_max",
                              "NetworkSelectorDatasetInfo:darts:layers",
                              "NetworkSelectorDatasetInfo:darts:init_channels", "epochs"]
    ...
    def _assert_diagonal(self, config):
        channel_fid = self.channel_fidelity_dict[config["NetworkSelectorDatasetInfo:darts:init_channels"]]
        cell_fid = self.cells_fidelity_dict[config["NetworkSelectorDatasetInfo:darts:layers"]]
        epoch_fid = self.epoch_fidelity_dict[config["SimpleLearningrateSchedulerSelector:cosine_annealing:T_max"]]
        return channel_fid == cell_fid == epoch_fid

    def _get_config_and_fidelity_hash(self, config):
        config_without_fidelities, fidelity_pars = self._separate_config_and_fidelities(config)
        return self._hash_dict_repr(config_without_fidelities), self._get_fidelity_number(fidelity_pars)

    def _separate_config_and_fidelities(self, config):
        config_without_fidelities = {k: v for k, v in config.items() if k not in self.fidelity_keys}
    ...
    def _get_fidelity_number(self, config):
        return self.channel_fidelity_dict[config["NetworkSelectorDatasetInfo:darts:init_channels"]]



class EdgeConfigDict():
    """
    Creates a config dict but with any fidelity allowed as fidelity hash 
    """

    def __init__(self, configs, accuracies, runtimes):
        ...
        self.data = self._create_data_dict(configs, accuracies, runtimes)

    def _create_data_dict(self, configs, accuracies, runtimes):
        data = dict()
        for config, accuracy, runtime in zip(configs, accuracies, runtimes):
            config_hash, fidelity_hash = self._get_config_and_fidelity_hash(config)
            if not config_hash in data.keys():
                data[config_hash] = dict()
            data[config_hash][fidelity_hash] = (config, accuracy, runtime)
        return data
    ...
    def _get_fidelity_number(self, config):
        ch = self.channel_fidelity_dict[config["NetworkSelectorDatasetInfo:darts:init_channels"]]
        c = self.cells_fidelity_dict[config["NetworkSelectorDatasetInfo:darts:layers"]]
        ep = self.epoch_fidelity_dict[config["SimpleLearningrateSchedulerSelector:cosine_annealing:T_max"]]
        return str(c)+str(ch)+str(ep)
