...
def analysis(run_name):
    ...
    budgets = []
    for conf in all_runs:
        if conf['info'] is not None and conf['loss'] is not None and conf['loss'] != np.nan and conf[
            'loss'] != np.inf and not math.isnan(conf.info[0]['test_kendall_tau']):
            loss = conf['loss']
            losses.append(loss)
            extra_corrs.append(1 - conf['info'][0]['test_kendall_tau'])
            budgets.append(conf['time_stamps']['finished'] - conf['time_stamps']['started'])
