...
class HPOWorker(Worker):
    ...
    def compute(self, config, budget, config_id, working_directory):
        ...
        if 'gnn' in self.model or 'bananas' in self.model:
            model_config['epochs'] = int(budget)
        elif 'lgb' in self.model or 'xgb' in self.model:
            model_config['param:num_rounds'] = int(budget)
        elif 'random_forrest' in self.model or 'sklearn_forest' in self.model or 'svr' in self.model or \
                'ngb' in self.model:
            print("Budget not considered for random forrests, ngb and svrs.")
        else:
        ...
        val_result = surrogate_model.train()
        ...
        convert_to_python_primitives = lambda input_dict: {k: getattr(v, "tolist", lambda: v)() for k, v in
                                                           input_dict.items()}
        # Create dict keys
        val_result = convert_to_python_primitives(self.change_key_prefix(val_result, "val"))
        test_result = convert_to_python_primitives(self.change_key_prefix(test_result, "test"))

        val_obj = val_result["val_kendall_tau"]
...
if __name__ == "__main__":
    worker = HPOWorker(eta=3, min_budget=5, max_budget=5, run_id=1, data_root='NasBench201_v0.4',
